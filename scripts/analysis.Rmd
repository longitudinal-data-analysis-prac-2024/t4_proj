...
```{r}
data.table::melt()
  #another function to reshape data from wide to long
  #melt(df, id.vars, variable.name, value.name)
  #id.vars is the outcome variable, variable.name is the names of 
  #the factors and value.name is the coloumn with the data to be tested, the scores 
  #melt(data = fancy.data, id.vars = c("pp", "intervention_type"), variable.name = 
  #"Domain_Type", value.name = "Effect_Score"))

library(tidyverse)

# adding NA values for the 2nd wave of data collection for grades
prep_data <- sum_data %>%
...
long_data <- prep_data %>%
  pivot_longer(
    cols = matches("C[123]_"), # Only pivot columns that match the pattern C1_, C2_, or C3_
...
# this has worked only the waves are in 1 3 2 order rather than 1 2 3 order which is a little annoying
...
#lmerTest::lmer(inflations ~ trial * condition + (1 | ID), data = adj.long)
