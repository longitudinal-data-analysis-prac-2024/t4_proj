...
grades_data <- long_data %>%
  select(ID, Time, Eng, Mat) %>%
  filter(Time == c("1", "3"))
...
grades_data %>%
  ...
  scale_x_continuous(breaks = c(1,2,3)) +
  scale_colour_manual(
    values = c("Eng" = "maroon4", "Mat" = "olivedrab3"), 
    name = "Subjects", 
    labels = c("English", "Maths"))



# ChatGTP test
# Prepare the data for plotting
grades_btotal_data <- long_data %>%
  pivot_longer(cols = c(Eng, Mat), names_to = "Subject", values_to = "Grade") %>%
  group_by(Time, Subject) %>%
  summarize(
  ...
  left_join(long_data %>% group_by(Time) %>% summarize(btotal_mean = mean(Btotal, na.rm = TRUE)), by = "Time")

# Plot the data
grades_btotal_data %>%
  ggplot(aes(x = Time)) +
  geom_boxplot(aes(y = btotal_mean), colour = "steelblue", linewidth = 1.5, linetype = "dashed") +  # Btotal line
  geom_boxplot(aes(y = grade_mean, colour = Subject, group = Subject), linewidth = 1.5) +  # Grade lines
  labs(title = "Grades and Btotal Over Time",
       y = "Mean Score",
       colour = "Measure") +
  scale_x_continuous(breaks = c(1, 2, 3)) +
  scale_colour_manual(
