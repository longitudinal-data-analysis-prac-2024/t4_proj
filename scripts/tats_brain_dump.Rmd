...
our_data <- our_data %>% 
  rowwise() %>%
  mutate(SDQ_T1 = sum(c_across(all_of(SDQ1) > 0) / length(SDQ1))
accuracy$Combined_Proportion <- rowMeans(select(accuracy, Proportion))
diabetes_use <- bind_cols(diabetes_use, accuracy$Combined_Proportion)
