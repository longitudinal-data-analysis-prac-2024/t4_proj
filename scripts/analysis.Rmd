...
# saving the new data frame because the imputation takes a hot minute so there's no need to re run it every time we open this scritp
...
ggplot(data = long_data[1:100], aes(x = Time, y = Btotal, colour = ID)) +
  geom_line() +
  theme_minimal () +
...
sem_model <- '
  # Measurement model
  Btotal =~ C1_Btotal + C2_Btotal + C3_Btotal
  AGtotal =~ AGtotal
  SCtotal =~ SCtotal
  LTtotal =~ LTtotal
  
  # Structural model
  Btotal ~ AGtotal + SCtotal + LTtotal + FSM + C1_Btotal + C2_Btotal + C3_Btotal
  
  # FSM affecting Btotal directly
  FSM ~~ Btotal
'

# fitting the model made
sem_fit1 <- sem(sem_model, data = sum_data, missing="fiml", fixed.x=FALSE)
